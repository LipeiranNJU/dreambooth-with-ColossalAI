...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        self.instance_images_path = [image for image in self.instance_images_path if ".png" in image.name or ".jpg" in image.name]
        print("李佩然测试:", self.instance_images_path)
        if test:
        ...
        if class_data_root is not None:
            ...
            self.class_images_path = [image for image in self.class_images_path if ".jpg" in image.name or ".png" in image.name]
        ...
        print("final 李佩然测试:", self.instance_images_path)
            ...
            if global_step % args.save_steps == 0:
                ...
                if local_rank == 0:
                    if not os.path.exists(os.path.join(save_path, "config.json")):
                        shutil.copy(os.path.join(args.output_dir, "config.json"), save_path)
